...
class Mackup(object):
    ...
    def get_apps_to_backup(self, apps_arg: Optional[List[str]] = None) -> Set[str]:
        """
        ...
        # Three options:
        #  - If user passed in apps as arguments, use those
        #  - If a list of apps to sync is specified in the config, use those
        #  - Otherwise, every supported app
        apps_to_backup = apps_arg or self._config.apps_to_sync or app_db.get_app_names()

        # Remove the specified apps to ignore, unless they were passed in via cli args
        if apps_arg is None:
            for app_name in self._config.apps_to_ignore:
                apps_to_backup.discard(app_name)

        utils.vlog(f"Backing up apps: {', '.join(apps_to_backup)}")
